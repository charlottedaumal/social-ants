    ...
    ,isStatsOn(false)
{
    ...
    mEnvBackground.setTexture(&getAppTexture(getAppConfig().simulation_day_background));
...
void Application::chooseBackground()
{
    ...
    if((getAppEnv().getDayCycle() == Day) or (getAppEnv().getTemperature() == getAppConfig().simulation_usual_temperature)) {
        mSimulationBackground.setTexture(&getAppTexture(isDebugOn() ?
                                         getAppConfig().simulation_debug_background :
                                         getAppConfig().simulation_day_background)
                                         , true);
    } else {
        mSimulationBackground.setTexture(&getAppTexture(isDebugOn() ?
                                         getAppConfig().simulation_night_background :
                                         getAppConfig().simulation_day_background)
                                         , true);
    }

    if(getAppEnv().getTemperature() == getAppConfig().simulation_low_temperature) {
        mSimulationBackground.setTexture(&getAppTexture(isDebugOn() ?
                                         getAppConfig().simulation_snow_background :
                                         getAppConfig().simulation_day_background)
                                         , true);
    }

    if(getAppEnv().getTemperature() == getAppConfig().simulation_high_temperature) {
        mSimulationBackground.setTexture(&getAppTexture(isDebugOn() ?
                                         getAppConfig().simulation_fire_background :
                                         getAppConfig().simulation_day_background)
                                         , true);
    }
